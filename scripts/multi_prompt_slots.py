    ...
    def ui(self, is_img2img):
        ...
        with gr.Column():
            ...
            gen_btn_clone.click(fn=None, _js=f"""() => {{
                ...
                const cloneGen = document.getElementById('multi_prompt_gen_clone');
                const cloneIntr = document.getElementById('multi_prompt_interrupt_clone');

                if (!realGen || !cloneGen) return;

                realGen.click();

                if (window.multiPromptSyncTimer) clearInterval(window.multiPromptSyncTimer);

                window.multiPromptSyncTimer = setInterval(() => {{
                    const m = document.getElementById('{target_gen_id}');
                    const i = document.getElementById('{target_intr_id}');
                    const cg = document.getElementById('multi_prompt_gen_clone');
                    const ci = document.getElementById('multi_prompt_interrupt_clone');
            ...
            def calculate_pure_prompt_images(raw_base_pos, raw_base_neg, gen_filter_text, main_only, inline_xyz_enabled, size_control_enabled, *prompts):
                ...
                return f"<span id='mp_calc_result' data-pure='{pure_images_per_batch}' style='color: #2ed573; font-weight: bold; font-size: 1.1em;'>{pure_images_per_batch} images</span>"
            ...
            check_count_btn.click(
                ...
                _js=f"""() => {{
                    ...
                    const resultSpan = document.getElementById('mp_calc_result');
